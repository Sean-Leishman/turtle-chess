...
class ZobristHasher:
    ...
    def hash_board(self, board: chess.BaseBoard) -> int:
        ...
        for pivot, squares in enumerate(board.color_occ[::-1]):
            for square in get_occupied_squares(squares)[::-1]:
                piece_index = board.get_piece_on(square) * 2 + pivot
                zobrist_hash ^= self.array[int(64 * piece_index + square)]
    ...
    def hash_turn(self, board: chess.Board) -> int:
        # Hash in the turn.
        return self.array[780] if board.color == Color.WHITE else 0
    ...
    def find_all(self, board: Union[chess.Board, int], *, minimum_weight: int = 1, exclude_moves: Container[chess.Move] = []) -> Iterator[Entry]:
        ...
        while i < size:
            ...
            if context:
                ...
                entry = Entry(entry.key, entry.raw_move, entry.weight, entry.learn, entry.move)
...
if __name__=="__main__":
    board = Board()
    board.initialise_boards()

    board2 = chess.Board()
    with open_reader("Perfect2017.bin") as reader:
